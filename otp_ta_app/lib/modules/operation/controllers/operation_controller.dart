...
class OperationController extends GetxController {
  ...
  void _watchPatients() {
    _patientRepository.watchAllPatients().listen(
      ...
      onError: (_) =>
          SnackbarHelper.showError('Error', 'Failed to load patients list.'),
    );
  }
  ...
  void _watchDoctors() {
    _doctorRepository.watchAllDoctors().listen(
      ...
      onError: (_) =>
          SnackbarHelper.showError('Error', 'Failed to load doctors list.'),
    );
  }
  ...
  bool isDoctorAvailable(String doctorId, DateTime scheduledDateTime) {
    ...
    final hasConflict = operationsList.any((op) {
      if (op.operationId == selectedOperation.value?.operationId) {
        return false; // Ignore current operation
      }
      if (op.status == OperationStatus.cancelled ||
          op.status == OperationStatus.completed) {
        return false;
      }
      if (op.surgicalTeam.primaryDoctorId != doctorId &&
          op.surgicalTeam.anaesthesiologistId != doctorId) {
        return false;
      }

      // If dates match and time is within, say, 2 hours (or same hour)
      final difference = op.scheduledDate
    ...
    result.fold(
      ...
      (operationId) {
        SnackbarHelper.showSuccess(
          'Success',
          'Operation Record Created Successfully',
        );
  ...
  Future<void> assignSurgicalTeam(
    String operationId,
    SurgicalTeamModel team,
  ) async {
    isLoading.value = true;
    final result = await _operationRepository.assignSurgicalTeam(
      operationId,
      team,
    );
    result.fold(
  ...
  Future<void> updateSurgicalTeam(
    ...
    final currentUserId =
        Get.find<AuthController>().currentUser.value?.uid ?? 'Admin';
    ...
    final result = await _operationRepository.updateSurgicalTeam(
      operationId,
      team,
      auditLog,
    );
    result.fold(
      ...
      (_) {
        SnackbarHelper.showSuccess(
          'Success',
          'Surgical Team Updated Successfully',
        );
        Get.back();
      },
    );
  ...
  Future<void> recordOutcome(
    ...
    isLoading.value = true;
    final result = await _operationRepository.recordOutcome(
      operationId,
      outcome,
    );
    result.fold(
      ...
      (_) async {
        SnackbarHelper.showSuccess(
          'Success',
          'Operation Outcome Recorded Successfully',
        );
  ...
  Future<void> uploadMedicalReport(File file, String operationId) async {
    isLoading.value = true;
    final result = await _operationRepository.uploadMedicalReport(
      file,
      operationId,
    );
    result.fold(
  ...
  void _triggerStatusNotification(
    String operationId,
    OperationStatus status,
  ) async {
    try {
      ...
      opRes.fold((_) {}, (operation) async {
        final patientDoc = await FirebaseFirestore.instance
            .collection('users')
            .doc(operation.patientId)
            .get();
        if (patientDoc.exists) {
          final patientUser = UserModel.fromMap(
            patientDoc.data()!,
            patientDoc.id,
          );
          ...
          if (fcmToken != null && fcmToken.isNotEmpty) {
            await RenderApiService.sendOperationStatusNotification(
              ...
              body:
                  'Your operation status has been updated to: ${status.name.toUpperCase()}',
      ...
      developer.log(
        'OperationController._triggerStatusNotification Exception',
        error: e,
    ...
    try {
      ...
      opRes.fold((_) {}, (operation) async {
        ...
        if (patientDoc.exists) {
          ...
          final tempPassword =
              'OTP_Temp_${DateTime.now().millisecondsSinceEpoch % 10000}';
          final deepLink =
              'https://otpta-app.web.app/reset-password?uid=$patientId';
      ...
      developer.log(
        'OperationController._triggerCredentialsDispatch Exception',
        error: e,
        name: 'OperationController',
      );
    }
