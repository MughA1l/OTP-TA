...
class RolePermissionRepositoryImpl implements IRolePermissionRepository {
  ...
  @override
  Future<Either<Failure, List<RolePermissionModel>>>
    ...
    try {
      ...
      if (snapshot.docs.isEmpty) {
        return Right([
          ...
          const RolePermissionModel(
            role: 'doctor',
            allowedModules: ['Dashboard', 'Patients', 'Procedures'],
          ),
          const RolePermissionModel(
            role: 'receptionist',
            allowedModules: ['Dashboard', 'Patients'],
          ),
          const RolePermissionModel(
            role: 'patient',
            allowedModules: ['Dashboard', 'Profile', 'Appointments'],
          ),
        ]);
      }

      final roles = snapshot.docs
          .map((doc) => RolePermissionModel.fromMap(doc.data(), doc.id))
          .toList();
      return Right(roles);
    } on FirebaseException catch (e) {
      return Left(
        FirestoreFailure(
          message: e.message ?? 'Failed to fetch role permissions.',
        ),
      );
    } catch (e) {
  ...
  Future<Either<Failure, void>> updateRolePermissions(
    ...
    try {
      await _firestore.collection('role_permissions').doc(role).set({
      ...
      return Left(
        FirestoreFailure(
          message: e.message ?? 'Failed to update role permissions.',
        ),
      );
    } catch (e) {
      return Left(FirestoreFailure(message: 'An unexpected error occurred.'));
