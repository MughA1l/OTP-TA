...
class ReportRepositoryImpl implements IReportRepository {
  ...
  Future<Either<Failure, AnalyticsCacheModel>> fetchOperationAnalytics(
    ...
    try {
      ...
      final allOps = querySnapshot.docs
      ...
      for (var op in filteredOps) {
        ...
        if (op.status == OperationStatus.completed) {
          ...
          final duration = diff > 0
              ? diff
              : 90; // Fallback to 90 min if duration is 0
          totalDuration += duration;
          completedWithDuration++;
        }
      }

      final avgDuration = completedWithDuration > 0
      ...
      for (var entry in doctorMap.entries) {
        ...
        for (var op in docOps) {
          ...
          final hasDelay = op.auditLog.any(
            (log) => log.action.toLowerCase().contains('delay'),
          );
          if (!hasDelay) {
            onTimeCount++;
          }
        ...
        final docAvgDuration = docCompletedCount > 0
            ? docDurationSum / docCompletedCount
            : 0.0;
        final punctuality = docOps.isNotEmpty
            ? onTimeCount / docOps.length
            : 1.0;
        ...
        doctorPerformances.add(
          DoctorPerformanceModel(
            ...
            doctorName: docOps
                .first
                .surgicalTeam
                .primaryDoctorId, // We will update names in controller/UI
      ...
      final resultCache = AnalyticsCacheModel(
        ...
        recoveryMetrics: {
          'readmissionRate': total > 0
              ? (filteredOps
                            .where(
                              (op) =>
                                  op.outcome?.complications
                                      .toLowerCase()
                                      .contains('readmit') ==
                                  true,
                            )
      ...
      return Left(
        FirestoreFailure(message: e.message ?? 'Failed to fetch analytics.'),
      );
    } catch (e) {
  ...
  Future<Either<Failure, DoctorPerformanceModel>> fetchDoctorPerformance(
    ...
    try {
      ...
      final avgDuration = completedCount > 0
          ? durationSum / completedCount
          : 0.0;
      ...
      return Left(
        FirestoreFailure(
          message: e.message ?? 'Failed to fetch doctor performance.',
        ),
      );
    } catch (e) {
      return Left(FirestoreFailure(message: 'An unexpected error occurred.'));
  ...
  Future<Either<Failure, Map<String, dynamic>>> fetchRecoveryAnalytics(
    ...
    try {
      ...
      final readmissionRate = filteredOps.isNotEmpty
          ? (readmissions / filteredOps.length) * 100
          : 0.0;
      ...
      return Left(
        FirestoreFailure(
          message: e.message ?? 'Failed to fetch recovery analytics.',
        ),
      );
    } catch (e) {
      return Left(FirestoreFailure(message: 'An unexpected error occurred.'));
  ...
  List<OperationModel> _applyFilters(
    List<OperationModel> ops,
    Map<String, dynamic> filters,
  ) {
    List<OperationModel> result = List.from(ops);

    if (filters.containsKey('otRoom') &&
        filters['otRoom'] != null &&
        filters['otRoom'].toString().isNotEmpty) {
      result = result.where((op) => op.otRoom == filters['otRoom']).toList();
    }

    if (filters.containsKey('surgeryType') &&
        filters['surgeryType'] != null &&
        filters['surgeryType'].toString().isNotEmpty) {
      result = result
          .where((op) => op.surgeryType == filters['surgeryType'])
          .toList();
    }
    ...
    if (filters.containsKey('endDate') && filters['endDate'] != null) {
      ...
      result = result
          .where(
            (op) =>
                op.scheduledDate.isBefore(endDate.add(const Duration(days: 1))),
          )
          .toList();
    }
